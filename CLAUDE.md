...
- **Funding-absent no-op = the dominant-case safety property (BYTE-IDENTICAL to the old behavior)**: when the FCY a buy spends has NO tracked acquisition lot in the data — the common real-file case: IBKR auto-convert (AFx) settlement, single-year Flex exports, FXCONV-skipped conversions — the buy parks "uncovered" (nothing to remove from an empty pool) and the matching sell re-adds at the SALE rate, which reproduces the previous full-proceeds result exactly. So nothing changes for the overwhelming majority of files; the carry-basis correction only bites on accounts that hold TRACKED FCY across multiple same-currency stock round-trips AND then convert to EUR.
- **Known limitation — loss-sell carried-principal residual (documented, accepted, now honestly sized — NOT a bug)**: any SELL that realizes a LOSS discards the carried principal it pulls back from the parked FIFO (the leg re-adds principal at its CARRIED basis but the loss means less value returns than the dollars were parked carrying), and the buy↔sale FX drift on that vanished principal is dropped — NOT only partial sells; any loss-sell that discards carried principal triggers it. Exact form: `residual = Σ over loss-sells of [ discardedPrincipalQty × (saleRate − carriedBasisRate) ]`. It is therefore PROPORTIONAL to the lost principal and the FX drift on it, and is **UNBOUNDED** — it scales to thousands or tens of thousands of euros on large accounts (demonstrated reaching **~€250,000** on a fund-$100k total-stock-loss case with the FX rate rising), not the −€20 a small-magnitude test happens to show. Its SIGN is rate-path-dependent: it UNDER-states the FX gain when the rate ROSE between buy and sale, OVER-states when it FELL — so it is not uniformly conservative in one direction. **Why it exists**: the genuine V2422-20 "two-element" gap — when the stock leg is a LOSS, principal exits the patrimony through the stock disposal, and the FX drift on that vanished principal is attributable to NEITHER the stock line (computed at a single sale-date rate, FX-stripped by design) NOR the divisa line (those dollars never reached a EUR conversión, so no Art. 14.2.e realization). It lives in the seam between the two elements V2422-20 deliberately separates. No DGT consulta resolves it, and forcing the raw-economic figure would itself DOUBLE-COUNT the FX drift against the already-recognized stock loss. The engine behaves per its spec — this note corrects only the magnitude characterization (the residual is large and unbounded, not "small/bounded"), not the engine. Accepted and documented, like the monodivisa `totalSavingsBase` limitation.
